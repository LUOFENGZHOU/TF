...
DCustomerintimacy=["marketing","advertis","brand","reputation","trademark","customer[a-z]*\s*service","consumer[a-z]*\s*service","customer[a-z]*\s*need","sales\s*support","post.purchase\s*service","customer\s*preference","c\s*preference","loyalty","customiz","personaliz","responsive"]
DHumanresources=["talent","train","skill","intellectual[a-z]*\s*propert","human\s*capital"]
CStrategicpositioning=["cost\s*leader","low[a-z]*\s*pric","low[a-z]*\s*cost","cost\s*advantage","aggressive\s*pric"]
CCostcontrol=["control[a-z]*\s*cost","control[a-z]*\s*expense","control[a-z]*\s*overhead","minimiz[a-z]*\s*cost","minimiz[a-z]*\s*expense","minimiz[a-z]*\s*overhead","reduc[a-z]*\s*cost","reduc[a-z]*\s*expense","reduc[a-z]*\s*overhead","sav[a-z]*\s*cost","improv[a-z]*\s*cost","cut[a-z]*\s*cost","cut[a-z]*\s*expense","cut[a-z]*\s*overhead","decrease[a-z]*\s*cost","decrease[a-z]*\s*expense","decrease[a-z]*\s*overhead","monitor[a-z]*\s*cost","monitor[a-z]*\s*expense","monitor[a-z]*\s*overhead"]
COperationefficiency=["efficien","high[a-z]*\s*yield","process[a-z]*\s*improvement","asset[a-z]*\s*utilization","capacity[a-z]*\s*utilization"]
...
DInd=np.zeros(4)
CInd=np.zeros(4)
filelist=glob.glob("*wordlist.csv")
def main(file):
    log={"Name":[],"Year":[],"CIK":[],"Date":[],"MainPart":[],"DS":[],"DP":[],"DC":[],"DH":[],"CS":[],"CC":[],"CO":[],"CE":[]}
    ...
    output.to_csv(str(year)+"_processed.csv")
