...
if __name__=='__main__':
    ...
    mainlist = pd.read_csv("out.csv", encoding = "utf-8")
    
    for i in range(len(mainlist["AlphaIndex"])):
        
        if os.path.exists(str(mainlist["AlphaIndex"][i]) + ".txt") == False: 
    ...
    #                page_art.close()
                    ...
                    with open(str(mainlist["AlphaIndex"][i]) + '.txt', "w", encoding = "utf-8") as f:
