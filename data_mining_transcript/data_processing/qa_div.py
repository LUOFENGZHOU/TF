...
def collect_essense(ind, mode):
    ...
    t1, t2 = re.findall(_item, delete_n(re.findall(strong_c, content))[0]), re.findall(_item, delete_n(re.findall(strong_c, content))[1])
...
#    ans_pattern = re.compile("<span class=\"answer\">")
#    qus_pattern = re.compile("<span class=\"question\">")
#    entity_pattern = re.compile("<p class=\"p p[0-9]?[0-9]\"><strong>(.*?)</strong></p>", re.M)
#    content_pattern = re.compile("</strong></p><p class=\"p p[0-9]?[0-9]\">(.*?)<p class=\"p p[0-9]?[0-9]\"><strong>", re.M|re.DOTALL)
    entity_pattern = re.compile("<p[^<>]*?>[^a-zA-Z<>]*<strong>(.*?)</strong>[^a-zA-Z<>]*</p>", re.M|re.DOTALL)   # a paragraph beginning with strong-tag
#    content_pattern = re.compile("</strong>[^a-zA-Z<>]*</p>[^a-zA-Z<>]*<p[^<>]*?\">(.*?)<p[^<>]*?\">[^a-zA-Z<>]*<strong>", re.M|re.DOTALL)
...
#        print(name)
        if os.path.exists(name) and not os.path.exists("./qapair/"+str(_mainlist["AlphaIndex"][i])+".tsv"): # rewrite-free mode
#        if os.path.exists(name): #rewrite mode
            ...
            content_list = []
            try:
                content_spt = re.split(entity_pattern, content)[1:] # splite the content when another entity exists. Note that the first match is always an empty string
#                content_spt = delete_em(content_spt) # this step should be dropped since there may exist an entity saying nothing.
            ...
            if flag == 1:
                pd.DataFrame([indl,indind, q_list, a_list]).transpose().to_csv("./qapair/"+str(_mainlist["AlphaIndex"][i])+".tsv", sep = "\t", encoding = "utf-8")
                d += len(indind)
                print("Success in " + str(_mainlist["AlphaIndex"][i]) + " !")
#            entity_list = re.findall(entity_pattern, content)
#            content_list = re.findall(content_pattern, content)
#            flag = 1
#            if len(entity_list) != len(content_list): # entity number = content number
#                log.push(_mainlist["AlphaIndex"][i], "Entity != Content")
#                print("Entity != Content")
#                flag = 0
#                continue
#            if len(re.findall(ans_pattern, content)) > 0: # content has predefined class
#                for j in range(len(entity_list)):
#                    if len(re.findall(qus_pattern, entity_list[j])) == 1:
#                        q_entity.append(re.sub("<span class=\"question\">", "", entity_list[j]))
#                        if j == len(entity_list) - 1: # question should not be the last one
#                            log.push(_mainlist["AlphaIndex"][i], "Question should not be the last one!")
#                            flag = 0
#                            print("Question should not be the last one!")
#                            break
#                        q_list.append(content_list[j])
#                    elif len(re.findall(ans_pattern, entity_list[j])) == 1:
#                        if len(q_list) == 0: # question exists prior to answer
#                            log.push(_mainlist["AlphaIndex"][i], "Answer should not be the first one!")
#                            flag = 0
#                            print("Answer should not be the first one!")
#                            break                    
#                        a_entity.append(re.sub("<span class=\"answer\">", "", entity_list[j]))
#                        a_list.append(content_list[j])  
#                if flag == 0: # meet an error in sub-for
#                    continue
#            else: # no predefined class, need referring raw data
#                log.push(_mainlist["AlphaIndex"][i], "No predefined class!")
#                flag = 0
#                print("No predefined class!")
#                continue
            
#        if flag == 1:
#            outname = str(_mainlist["AlphaIndex"][i]) + "_qa_part_sep" + ".csv"
#            pd.DataFrame([q_entity, q_list, a_entity, a_list]).transpose().to_csv("./sep/"+outname)
    print(str(d))
    log.out("error.log")